...
def record(self, motion):
    ...
    if motion == "record":
        test_df.loc[len(test_df)] = [len(test_df),self.Relbow_list, self.Rshoulder_list, self.Rbody_list, self.Rknee_list]
        test_df.to_csv(r"opencv\shooting_detect\train_data\test.csv", index=False)  # 將更改寫入到 CSV 檔案中